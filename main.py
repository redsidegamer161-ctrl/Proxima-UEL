...
def check_and_download_font():
    if not os.path.exists("font.ttf"):
        ...
        try:
            # Use Bebas Neue from Google Fonts (regular weight)
...
# --- MASTER CARD GENERATOR (with bigger font) ---
async def generate_transaction_card(player, team_name, team_color, title_text="OFFICIAL SIGNING", custom_bg_url=None):
    ...
    # 5. Text (Bebas Neue with larger sizes)
    try:
        # Increase sizes: player name 80, title 50
...
class LeagueBot(discord.Client):
    ...
    async def on_ready(self):
        # Sync commands globally (safe for a bot in only one server)
...
# --- STARTUP ---
print("System: Loading Proxima V17 (Bebas Neue Font)...")
if TOKEN:
    try:
        keep_alive()
        client.run(TOKEN)
